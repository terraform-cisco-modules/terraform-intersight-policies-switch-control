...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-sw-ctrl-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} Switch Control Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/SwitchControlPolicies/%s", moid), expectedJSONTemplate, vars)
}
